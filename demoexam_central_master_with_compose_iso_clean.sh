...
1) Открой консоль HQ-RTR.
2) Определи порт к ISP:
   show interfaces status
   В Proxmox смотри MAC: HQ-RTR → Hardware → Network Device.
   Нужен интерфейс, который подключен к сети ISP-HQ.

Обычно это gigabitethernet 1/0/2.
Если другой — замени 1/0/2 в блоке ниже.
...
1) Открой консоль BR-RTR.
2) Определи порт к ISP:
   show interfaces status
   В Proxmox смотри MAC: BR-RTR → Hardware → Network Device.
   Нужен интерфейс, который подключен к сети ISP-BR.

Обычно это gigabitethernet 1/0/3.
Если другой — замени 1/0/3 в блоке ниже.
...
c "exit"
c "ruleset SNAT"
c "to interface gigabitethernet 1/0/2"
c "rule 1"
...
c "exit"
c "ruleset SNAT"
c "to interface gigabitethernet 1/0/3"
c "rule 1"
...
nmcli con add type ethernet ifname "$IF" con-name HQ-SRV ipv4.addresses 192.168.100.2/27 ipv4.gateway 192.168.100.1 ipv4.method manual 2>/dev/null || true
...
IF=$(ip -o link show | awk -F': ' '$2!="lo"{print $2; exit}')
nmcli con add type ethernet ifname "$IF" con-name BR-SRV ipv4.addresses 192.168.30.2/28 ipv4.gateway 192.168.30.1 ipv4.method manual 2>/dev/null || true
...
IF=$(ip -o link show | awk -F': ' '$2!="lo"{print $2; exit}')
nmcli con add type ethernet ifname "$IF" con-name HQ-CLI ipv4.addresses 192.168.200.3/28 ipv4.gateway 192.168.200.1 ipv4.method manual 2>/dev/null || true
...
main(){
  ...
  say "Центральная пошаговая настройка + compose.zip + Additional.iso для BR-SRV с видимым прогрессом. VESR: только commit + confirm, без save."
