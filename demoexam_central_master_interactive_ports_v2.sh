...
1. В Proxmox смотри ВМ → Оборудование → Сетевое устройство.
2. Смотри bridge/vmbr и MAC.
3. На Eltex выполни:
   show interfaces status
4. Сравни MAC из Proxmox с MAC в Eltex.
5. Вводить в скрипт можно ТОЛЬКО последнюю цифру порта:
   2 = gigabitethernet 1/0/2
   3 = gigabitethernet 1/0/3

Твоя карта мостов Proxmox:
  vmbr0    = WAN / Интернет
  vmbr1018 = ISP-HQ
  vmbr1019 = ISP-BR
  vmbr1020 = HQ-SRV-Net
...
Для HQ-RTR обычно:
  порт с bridge vmbr1018 = ISP-HQ
  порт с bridge vmbr1020 = HQ-SRV-Net
  порт с bridge vmbr1021 = HQ-CLI-Net

Для BR-RTR обычно:
  порт с bridge vmbr1022 = BR-Net / BR-LAN
  порт с bridge vmbr1019 = ISP-BR
...
ask_topology(){
  say ""
  say "Укажи карту стенда. Enter = значение по умолчанию."
  say "На ISP интерфейсы Linux смотри командой: ip -br a"
  say "У тебя обычно:"
  say "  enp7s1 = vmbr0 / WAN / Интернет"
  say "  enp7s2 = vmbr1018 / ISP-HQ"
  say "  enp7s3 = vmbr1019 / ISP-BR"
  say ""

  WAN_IF=$(ask_default "ISP: WAN-интерфейс в Интернет, bridge vmbr0" "${WAN_IF:-enp7s1}")
  HQ_IF=$(ask_default "ISP: интерфейс к HQ-RTR, bridge vmbr1018 / ISP-HQ" "${HQ_IF:-enp7s2}")
  BR_IF=$(ask_default "ISP: интерфейс к BR-RTR, bridge vmbr1019 / ISP-BR" "${BR_IF:-enp7s3}")

  print_bridge_map_hint

  say "Порты HQ-RTR."
  say "Вводи только последнюю цифру: например 2 вместо gigabitethernet 1/0/2."
  HQ_RTR_ISP_PORT=$(ask_port_num "HQ-RTR: порт к ISP-HQ / vmbr1018" "${HQ_RTR_ISP_NUM:-2}")
  HQ_RTR_SRV_PORT=$(ask_port_num "HQ-RTR: порт к HQ-SRV-Net / vmbr1020" "${HQ_RTR_SRV_NUM:-3}")
  HQ_RTR_CLI_PORT=$(ask_port_num "HQ-RTR: порт к HQ-CLI-Net / vmbr1021" "${HQ_RTR_CLI_NUM:-4}")
  HQ_RTR_MGMT_PORT=$(ask_port_num "HQ-RTR: порт к HQ-MGMT, если есть; если нет — Enter" "${HQ_RTR_MGMT_NUM:-5}")

  say ""
  say "Порты BR-RTR."
  say "BR-LAN = BR-Net = bridge vmbr1022."
  say "ISP-BR = bridge vmbr1019."
  say "Вводи только последнюю цифру."
  BR_RTR_LAN_PORT=$(ask_port_num "BR-RTR: порт к BR-Net / BR-LAN / vmbr1022" "${BR_RTR_LAN_NUM:-2}")
  BR_RTR_ISP_PORT=$(ask_port_num "BR-RTR: порт к ISP-BR / vmbr1019" "${BR_RTR_ISP_NUM:-3}")

  say ""
  say "Linux-интерфейсы. Смотри внутри каждой ВМ: ip -br a"
  HQ_SRV_IF=$(ask_default "HQ-SRV: интерфейс в HQ-SRV-Net / vmbr1020" "${HQ_SRV_IF:-enp7s1}")
  BR_SRV_IF=$(ask_default "BR-SRV: интерфейс в BR-Net / vmbr1022" "${BR_SRV_IF:-enp7s1}")
  HQ_CLI_IF=$(ask_default "HQ-CLI: интерфейс в HQ-CLI-Net / vmbr1021" "${HQ_CLI_IF:-enp7s1}")
  ...
  say "  ISP-HQ       = $HQ_IF"
  say "  ISP-BR       = $BR_IF"
  say "  HQ-RTR ISP   = $HQ_RTR_ISP_PORT"
  say "  HQ-RTR SRV   = $HQ_RTR_SRV_PORT"
  say "  HQ-RTR CLI   = $HQ_RTR_CLI_PORT"
  say "  HQ-RTR MGMT  = $HQ_RTR_MGMT_PORT"
  say "  BR-RTR LAN   = $BR_RTR_LAN_PORT"
  say "  BR-RTR ISP   = $BR_RTR_ISP_PORT"
  say "  HQ-SRV IF    = $HQ_SRV_IF"
  say "  BR-SRV IF    = $BR_SRV_IF"
  say "  HQ-CLI IF    = $HQ_CLI_IF"
  say ""
  read -rp "Продолжить с этой картой? yes/no [yes]: " okmap
  okmap="${okmap:-yes}"
  [ "$okmap" = "yes" ] || return 1
}
