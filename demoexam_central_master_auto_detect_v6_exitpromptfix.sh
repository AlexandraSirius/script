...
detect_router_ports_from_bootstrap(){
  ...
  HQ_RTR_ISP_PORT="$(port_by_ip_in_run "$hqrun" "172.16.1.2/28")"
  HQ_RTR_SRV_PORT="$(port_by_ip_in_run "$hqrun" "192.168.100.1/27")"
  HQ_RTR_CLI_PORT="$(port_by_ip_in_run "$hqrun" "192.168.200.1/28")"
  ...
  BR_RTR_ISP_PORT="$(port_by_ip_in_run "$brrun" "172.16.2.2/28")"
  BR_RTR_LAN_PORT="$(port_by_ip_in_run "$brrun" "192.168.30.1/28")"
...
main(){
  ...
  say "Центральная пошаговая настройка + авто через nmcli/ip + show running-config + fix без default-expired + HQ-MGMT удалён + Eltex post-check warning only + compose.zip + Additional.iso с видимым прогрессом. VESR: только commit + confirm, без save."
